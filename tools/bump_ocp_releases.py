...
def get_docker_client():
    if sh.which("podman") is not None:
        return sh.podman
    elif sh.which("docker") is not None:
        return sh.docker

    raise RuntimeError("You must have either podman or docker installed")


def verify_latest_config(clone_dir):
    client = get_docker_client()

    client.build(clone_dir, file=clone_dir / "Dockerfile.assisted-service-build",
                 tag="assisted-service-build")
    try:
        client.run("assisted-service-build", "make", "generate-configuration")
    except sh.ErrorReturnCode_2:
        # We run the command just for its side-effects, we don't care if it fails
        pass

    client.run("assisted-service-build", "make", "generate-bundle")
...
def bump_ocp_releases(username, password, dry_run):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        verify_latest_config(clone_dir)
...
def update_release_images_json(default_release_images_json, updates_made, updates_made_str, dry_run, clone_dir):
    ...
    if updates_made:
        with (clone_dir / DEFAULT_RELEASE_IMAGES_FILE).open("w") as outfile:
...
def update_os_images_json(default_os_images_json, updates_made, updates_made_str, dry_run, clone_dir):
    ...
    if updates_made:
        with (clone_dir / DEFAULT_OS_IMAGES_FILE).open("w") as outfile:
            json.dump(updated_version_json, outfile, indent=4)
